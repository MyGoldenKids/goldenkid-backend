...
public class FileController {
    ...
    @PostMapping("/upload/flist/{fileListId}/{memberId}")
    public ResponseEntity<Map<String, Object>> saveFilesByFileListId(@RequestPart List<MultipartFile> files, @PathVariable int fileListId, @PathVariable int memberId) {
        int retValue = fileService.saveFilesByFileListId(files, fileListId, memberId);
        if (retValue != 0) return ResponseResource.handleSuccess(retValue, "파일 업로드 성공!");
        else return ResponseResource.handleError("파일 업로드 실패..");
    }
